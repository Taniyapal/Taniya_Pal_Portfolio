...
axis(1, at=1:20)
##We can see the RMSE is lowest at k=3. So, we will take k=3.
...
for (i in 1:100){
  ...
  model.fit <- caret::train(TVC ~ ., method='knn', data=trainTransformed,k=3)
  test.predictions <- predict(model.fit, testTransformed)
...
#We got mtry=7 as the optimum hyperparameter after tuning

model.fit <- randomForest(TVC ~ ., data = TVC_data, mtry = 7,
                         importance = TRUE, na.action = na.omit)
plot(model.fit, main="Error over the number of trees")
test.predictions <- predict(model.fit, testTransformed)
model.rmse <- RMSE(testTransformed$TVC, test.predictions)
...
set.seed(90)
train.index <- createDataPartition(Pseu_data$TVC, p = .7,
                                   list = FALSE, times = 1)
...
plot(model.fit$results$k, model.fit$results$RMSE, xaxt="n",
     ylab="RMSE", xlab="k", main="RMSE for k-Nearest Neighbours regression")
axis(1, at=1:20)
##We can see the RMSE is lowest at k=2. So, we will take k=2.
...
#We got mtry=11 as the optimum hyperparameter after tuning

model.fit <- randomForest(Pseudomonads ~ ., data = Pseu_data, mtry = 11,
                          importance = TRUE, na.action = na.omit)
